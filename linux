...
However, Chromium can't be run as root because it runs in its own sandbox and it's hard (impossible?) to hide stuff from root and root from stuff. Sandbox can be disabled, but why do you want to leave your browser unprotected? I find it strange even when restoring VM from snapshot after each shutdown.
...
where `chromiumUserName` is the name of the user you created to run chromium.
